...
func (op Jri) Op_get_desc() string {
	return "Register indirect Jump to a program location"
}

func (op Jri) Op_show_assembler(arch *Arch) string {
	opBits := arch.Opcodes_bits()
	result := "jri [" + strconv.Itoa(int(arch.R)) + "(Reg)]	// Jump to the location contained in reg [" + strconv.Itoa(opBits+int(arch.R)) + "]\n"
	return result
}
...
func (op Jri) Op_instruction_verilog_state_machine(conf *Config, arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		result += NextInstruction(conf, arch, 6, "_"+strings.ToLower(Get_register_name(i)))
		result += "							$display(\"JRI " + strings.ToUpper(Get_register_name(i)) + "\");\n"
		result += "						end\n"
	}
	result += "						endcase\n"
	result += "					end\n"
...
func (Op Jri) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq) > 0 && types == O_REGISTER {
		...
		newnot0 := UsageNotify{C_OPCODE, "jri", I_NIL}
		result[0] = newnot0
...
func (Op Jri) HLAssemblerMatch(arch *Arch) []string {
	...
	result = append(result, "jri::*--type=reg")
	result = append(result, "jmp::*--type=loc--locaddressing=register")
	return result
}
...
func (Op Jri) HLAssemblerNormalize(arch *Arch, rg *bmreqs.ReqRoot, node string, line *bmline.BasmLine) (*bmline.BasmLine, error) {
	...
	case "jmp":
		regNeed := line.Elements[0].GetMeta("locregister")
		rg.Requirement(bmreqs.ReqRequest{Node: node, T: bmreqs.ObjectSet, Name: "registers", Value: regNeed, Op: bmreqs.OpAdd})
		line.Operation.SetValue("jri")
		line.Elements[0].SetValue(regNeed)
		line.Elements[0].RmMeta("locregister")
		line.Elements[0].RmMeta("locaddressing")
