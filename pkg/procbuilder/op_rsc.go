...
func (op Rsc) Op_show_assembler(arch *Arch) string {
	...
	result := "rsc [" + strconv.Itoa(int(arch.R)) + "(Reg)] [" + strconv.Itoa(int(arch.R)) + "(Reg)]	// Set a register to the sum of its value with another register [" + strconv.Itoa(opbits+int(arch.R)+int(arch.R)) + "]\n"
	return result
}
...
func (op Rsc) Op_instruction_verilog_state_machine(arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		for j := 0; j < reg_num; j++ {
			...
			result += "								$display(\"RSC " + strings.ToUpper(Get_register_name(j)) + " " + strings.ToUpper(Get_register_name(i)) + "\");\n"
			result += "							end\n"
...
func (op Rsc) Simulate(vm *VM, instr string) error {
	...
	case 8:
		vm.Registers[regdest] = vm.Registers[regsrc].(uint8) - vm.Registers[regdest].(uint8)
	case 16:
		vm.Registers[regdest] = vm.Registers[regsrc].(uint16) - vm.Registers[regdest].(uint16)
	default:
		// TODO Fix
	}
	vm.Pc = vm.Pc + 1
...
func (Op Rsc) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq0) > 0 && types0 == O_REGISTER && len(seq1) > 0 && types1 == O_REGISTER {
		...
		newnot0 := UsageNotify{C_OPCODE, "rsc", I_NIL}
		result[0] = newnot0
