...
func (op Xnor) Op_get_desc() string {
	return "Register bitwise xnor"
}

func (op Xnor) Op_show_assembler(arch *Arch) string {
	opbits := arch.Opcodes_bits()
	result := "xnor [" + strconv.Itoa(int(arch.R)) + "(Reg)] [" + strconv.Itoa(int(arch.R)) + "(Reg)]	// Set a register to the logical and of its value with another register [" + strconv.Itoa(opbits+int(arch.R)+int(arch.R)) + "]\n"
	return result
}
...
func (op Xnor) Op_instruction_verilog_state_machine(arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		for j := 0; j < reg_num; j++ {
			...
			result += "								$display(\"XNOR " + strings.ToUpper(Get_register_name(i)) + " " + strings.ToUpper(Get_register_name(j)) + "\");\n"
			result += "							end\n"
...
func (Op Xnor) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq0) > 0 && types0 == O_REGISTER && len(seq1) > 0 && types1 == O_REGISTER {
		...
		newnot0 := UsageNotify{C_OPCODE, "xnor", I_NIL}
		result[0] = newnot0
