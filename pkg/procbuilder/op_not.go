...
func (op Not) Op_instruction_verilog_state_machine(arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		for j := 0; j < reg_num; j++ {
			...
			result += "								$display(\"NOT " + strings.ToUpper(Get_register_name(j)) + "\");\n"
			result += "							end\n"
...
func (op Not) Simulate(vm *VM, instr string) error {
	...
	case 8:
		vm.Registers[regdest] = ^vm.Registers[regsrc].(uint8)
	case 16:
		vm.Registers[regdest] = ^vm.Registers[regsrc].(uint16)
	default:
		// TODO Fix
	}
	vm.Pc = vm.Pc + 1
...
func (Op Not) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq0) > 0 && types0 == O_REGISTER && len(seq1) > 0 && types1 == O_REGISTER {
		...
		newnot0 := UsageNotify{C_OPCODE, "not", I_NIL}
		result[0] = newnot0
