...
func (op Chw) Op_get_desc() string {
	return "Channel operation wait"
}

func (op Chw) Op_show_assembler(arch *Arch) string {
	opbits := arch.Opcodes_bits()
	result := "chw [" + strconv.Itoa(int(arch.R)) + "(Reg)]	// Wait for a channel operation [" + strconv.Itoa(opbits+int(arch.R)) + "]\n"
	return result
}
...
func (op Chw) OpInstructionVerilogHeader(conf *Config, arch *Arch, flavor string, pname string) string {
	...
	for _, currop := range arch.Op {
		if currop.Op_get_name() == "chc" {
			setflag = false
			break
		} else if currop.Op_get_name() == "chw" {
			break
		}
	}
	if setflag {
		...
		result += "\treg [" + strconv.Itoa(channel_num-1) + ":0] count_ready;\n"
...
func (op Chw) Op_instruction_verilog_state_machine(arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
