...
func (op Cilc) Op_get_desc() string {
	return "Left Shift of a register with control of carry flag"
}

func (op Cilc) Op_show_assembler(arch *Arch) string {
	opbits := arch.Opcodes_bits()
	result := "cilc [" + strconv.Itoa(int(arch.R)) + "(Reg)]	// Left Shift with control of carry-flag [" + strconv.Itoa(opbits+int(arch.R)) + "]\n"
	return result
}
...
func (op Cilc) Op_instruction_verilog_state_machine(arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		result += "							$display(\"Cilc " + strings.ToUpper(Get_register_name(i)) + "\");\n"
		result += "						end\n"
	}
	result += "						endcase\n"
...
func (Op Cilc) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq) > 0 && types == O_REGISTER {
		...
		newnot0 := UsageNotify{C_OPCODE, "cilc", I_NIL}
		result[0] = newnot0
