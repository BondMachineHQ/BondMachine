...
func (op Cir) Op_show_assembler(arch *Arch) string {
	...
	result := "cir [" + strconv.Itoa(int(arch.R)) + "(Reg)] 	// Set a register to the logical and of its value with another register [" + strconv.Itoa(opbits+int(arch.R)) + "]\n"
	return result
}
...
func (op Cir) Op_instruction_verilog_state_machine(arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		result += "								$display(\"CIR " + strings.ToUpper(Get_register_name(i)) + "\");\n"
		result += "						end\n"
	}
	result += "						endcase\n"
...
func (Op Cir) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq) > 0 && types == O_REGISTER {
		...
		newnot0 := UsageNotify{C_OPCODE, "cir", I_NIL}
		result[0] = newnot0
