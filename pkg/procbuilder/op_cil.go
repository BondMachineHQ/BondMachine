...
func (op Cil) Op_get_desc() string {
	return "Register left shift"
}

func (op Cil) Op_show_assembler(arch *Arch) string {
	opbits := arch.Opcodes_bits()
	result := "cil [" + strconv.Itoa(int(arch.R)) + "(Reg)] 	// Set a register to the logical and of its value with another register [" + strconv.Itoa(opbits+int(arch.R)) + "]\n"
	return result
}
...
func (op Cil) Op_instruction_verilog_state_machine(arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		result += "								$display(\"CIL " + strings.ToUpper(Get_register_name(i)) + "\");\n"
		result += "						end\n"
	}
	result += "						endcase\n"
...
func (Op Cil) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq) > 0 && types == O_REGISTER {
		...
		newnot0 := UsageNotify{C_OPCODE, "cil", I_NIL}
		result[0] = newnot0
