...
func (op Q2r) getQueueName(queueId int) string {
...
func (op Q2r) Op_get_desc() string {
	return "Get a value from a shared queue and put it in a register"
}
...
func (op Q2r) Op_show_assembler(arch *Arch) string {
	...
	result := "q2r [" + strconv.Itoa(int(arch.R)) + "(Reg)] [ " + strconv.Itoa(int(queueBits)) + " (Shared Queue)]	// " + op.Op_get_desc() + " [" + strconv.Itoa(opBits+int(arch.R)+queueBits) + "]\n"
	return result
}
...
func (op Q2r) Op_instruction_verilog_state_machine(conf *Config, arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	if queueNum > 0 {
		...
		for i := 0; i < reg_num; i++ {
			...
			if queueBits == 1 {
				result += "							case (current_instruction[" + strconv.Itoa(rom_word-opBits-queueBits-1) + "])\n"
			} else {
				result += "							case (current_instruction[" + strconv.Itoa(rom_word-opBits-queueBits-1) + ":" + strconv.Itoa(rom_word-opBits-int(arch.R)-int(queueBits)) + "])\n"
			}
			...
			for j := 0; j < queueNum; j++ {
				...
				result += "								end\n"
				result += "								$display(\"T2R " + strings.ToUpper(Get_register_name(i)) + " " + strings.ToUpper(op.getQueueName(j)) + "\");\n"
				result += "							end\n"
...
func (Op Q2r) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	newnot := UsageNotify{C_OPCODE, "q2r", I_NIL}
	result[0] = newnot
	return result, nil
}
...
func (Op Q2r) HLAssemblerMatch(arch *Arch) []string {
	...
	result = append(result, "q2r::*--type=reg::*--type=somov--sotype=q")
	result = append(result, "mov::*--type=reg::*--type=somov--sotype=q")
	return result
}
