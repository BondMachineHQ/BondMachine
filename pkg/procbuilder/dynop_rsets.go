...
func (op Rsets) Op_get_desc() string {
	return "Register set value with fixed size"
}

func (op Rsets) Op_show_assembler(arch *Arch) string {
	opbits := arch.Opcodes_bits()
	result := op.rsetsName + " [" + strconv.Itoa(int(arch.R)) + "(Reg)] [" + strconv.Itoa(op.s) + "(Value)]	// Set a register to the given value [" + strconv.Itoa(opbits+int(arch.R)+int(op.s)) + "]\n"
	return result
}

func (op Rsets) Op_get_instruction_len(arch *Arch) int {
	opbits := arch.Opcodes_bits()
	return opbits + int(arch.R) + op.s // The bits for the opcode + bits for a register + fixed size bit
}
...
func (op Rsets) Op_instruction_verilog_state_machine(conf *Config, arch *Arch, rg *bmreqs.ReqRoot, flavor string) string {
	...
	result := ""
	result += "					" + strings.ToUpper(op.rsetsName) + ": begin\n"
	if arch.R == 1 {
	...
	for i := 0; i < regNum; i++ {
		...
		result += "							_" + strings.ToLower(Get_register_name(i)) + " <= #1 current_instruction[" + strconv.Itoa(romWord-opBits-1-int(arch.R)) + ":" + strconv.Itoa(romWord-opBits-op.s-int(arch.R)) + "];\n"
...
func (op Rsets) Assembler(arch *Arch, words []string) (string, error) {
	...
	if partial, err := Process_number(words[1]); err == nil {
		result += zeros_prefix(sSize, partial)
	} else {
		return "", Prerror{err.Error()}
	}

	for i := opBits + int(arch.R) + sSize; i < romWord; i++ {
		result += "0"
	}
...
func (op Rsets) Disassembler(arch *Arch, instr string) (string, error) {
	...
	value := get_id(instr[arch.R : int(arch.R)+sSize])
