...
func (op R2owa) Op_show_assembler(arch *Arch) string {
	...
	result := "r2owa [" + strconv.Itoa(int(arch.R)) + "(Reg)] [" + strconv.Itoa(outbits) + "(Output)]	// Set an output to the value of a given register [" + strconv.Itoa(opbits+int(arch.R)+outbits) + "]\n"
	return result
}
...
func (op R2owa) Simulate(vm *VM, instr string) error {
	...
	outp := get_id(instr[regBits : int(regBits)+outBits])
	vm.Outputs[outp] = vm.Registers[reg]
	vm.OutputsValid[outp] = true
	if vm.OutputsRecv[outp] {
		vm.OutputsValid[outp] = false
		vm.Pc = vm.Pc + 1
	}
	return nil
}
...
func (Op R2owa) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq0) > 0 && types0 == O_REGISTER && len(seq1) > 0 && types1 == O_OUTPUT {
		...
		newnot0 := UsageNotify{C_OPCODE, "r2owa", I_NIL}
		result[0] = newnot0
...
func (Op R2owa) HLAssemblerMatch(arch *Arch) []string {
	...
	result = append(result, "r2owa::*--type=reg::*--type=output")
	result = append(result, "mov--iomode=sync::*--type=output::*--type=reg")
	return result
}
