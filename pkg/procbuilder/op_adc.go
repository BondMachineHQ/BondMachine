...
func (op Adc) Op_show_assembler(arch *Arch) string {
	...
	result := "adc [" + strconv.Itoa(int(arch.R)) + "(Reg)] [" + strconv.Itoa(int(arch.R)) + "(Reg)]	// Set a register to the sum of its value with another register [" + strconv.Itoa(opbits+int(arch.R)+int(arch.R)) + "]\n"
	return result
}
...
func (op Adc) Op_instruction_verilog_state_machine(arch *Arch, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		for j := 0; j < reg_num; j++ {
			...
			result += "								$display(\"ADC " + strings.ToUpper(Get_register_name(i)) + " " + strings.ToUpper(Get_register_name(j)) + "\");\n"
			result += "							end\n"
...
func (Op Adc) AbstractAssembler(arch *Arch, words []string) ([]UsageNotify, error) {
	...
	if len(seq0) > 0 && types0 == O_REGISTER && len(seq1) > 0 && types1 == O_REGISTER {
		...
		newnot0 := UsageNotify{C_OPCODE, "adc", I_NIL}
		result[0] = newnot0
