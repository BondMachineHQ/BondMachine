...
const (
	aximmPynqExample = `
	{
		...
		"source": [
			...
			"\n",
			"dataset_info = {\n",
			"    \"preloaded\": False, # set to True if you want to use a dataset, if set to False the dataset is generated randomly\n",
		...
		"source": [
			...
			"                    print(\"Unsigned received from bm is: \", raw_res)\n",
			"                    print(\"Binary received from bm is:   \", bin_res)\n",
			"                    output = cast_binary_to_float(bin_res_str)\n",
			"                    print(\"Output casted with bmnumbers is: \", output)\n",
			"                else:\n",
		...
		"source": [
			...
			"else:\n",
			"    X_test = np.random.uniform(-20, 20, size=1).reshape(-1)\n",
